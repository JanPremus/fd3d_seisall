! Evaluates synthetic seismograms for model in mtilde.dat
! with DWN Green's functions from NEZsor.dat.
! Coded by Frantisek Gallovic (2018)
!-------------------------------------------------------------------------------
    ...
    INTEGER NLgf,NWgf
    REAL*8 T,tw0,artifDT,leng,widt,epicL,epicW
    REAL*8,ALLOCATABLE,DIMENSION(:):: fc1,fc2
    REAL*8 elem,dL,dW,dt,df,Tshift
    ...
    real*8 dum
    INTEGER i,j,jj,k,jw,jl,mm,dumi,ifc
    INTEGER np_source
...
!$omp end parallel do
    ...
    open(393,FILE='stainfo.dat')
    do jj=1,NR
      write(*,*)'Station ',jj
      read(393,*)dum,dum,dum,dum,dum,dum,ifc
      j=0
      read(20) dumi
      do jw=1,NWgf
        do jl=1,NLgf
          ...
          do k=1,nfmax
            read(20) dumarr
            if(k>np)cycle
            cirN(k,j)=cmplx(dumarr(1),dumarr(4))*mu(jl,jw)*elem*exp(cmplx(0.,2.d0*PI*(k-1)*df*Tshift))
            cirE(k,j)=cmplx(dumarr(2),dumarr(5))*mu(jl,jw)*elem*exp(cmplx(0.,2.d0*PI*(k-1)*df*Tshift))
            cirZ(k,j)=cmplx(dumarr(3),dumarr(6))*mu(jl,jw)*elem*exp(cmplx(0.,2.d0*PI*(k-1)*df*Tshift))
          enddo
        enddo
      enddo
      dtr4=dt
